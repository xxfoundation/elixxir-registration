...
//Happy path
func TestRegistrationImpl_PollPartialNdf(t *testing.T) {
	//Create database
	var err error
	storage.PermissioningDb, _, err = storage.NewDatabase("test",
		"password", "regCodes", "0.0.0.0", "-1")
	...
	l.Lock()
	observedNDFBytes, err := impl.PollPartialNdf(nil, &connect.Auth{})
	l.Unlock()
	if err != nil {
		t.Errorf("failed to update ndf: %v", err)
	}
...
//Error  path
func TestRegistrationImpl_PollPartialNdf_NoNDF(t *testing.T) {
	//Create database
	var err error
	storage.PermissioningDb, _, err = storage.NewDatabase("test",
		"password", "regCodes", "0.0.0.0", "-1")
	...
	_, err = impl.PollPartialNdf(clientNdfHash, &connect.Auth{})
	if err == nil {
		t.Error("Expected error path, should not have an ndf ready")
	}
	if err != nil && err.Error() != ndf.NO_NDF {
